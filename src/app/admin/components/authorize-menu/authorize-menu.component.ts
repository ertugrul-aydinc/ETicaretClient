...
import { Menu } from 'src/app/contracts/application_configurations/menu';
...
export class AuthorizeMenuComponent extends BaseComponent implements OnInit{
  ...
  async ngOnInit(){
    this.dataSource.data = await (await this.applicationService.getAuthorizedDefinitionEndpoints()).map(m =>{
